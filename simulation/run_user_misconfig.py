...
# ---------- Solar & Load Models (baseline-style) ----------
...
def simulate_room_base_loads(minute_of_day: int):
    ...
    Same pattern as baseline but kitchen is slightly heavier,
    so misconfigured threshold hurts more.
    """
    hour = minute_of_day // 60

    bedroom = 0.05
    hall = 0.05
    kitchen = 0.10  # a bit higher base than others
...
def add_kitchen_spikes(minute_of_day: int, bedroom_kw: float, hall_kw: float, kitchen_kw: float):
    """
    Realistic kitchen spikes (mixer):
    ...
    if in_window(7, 10, 14) or in_window(13, 0, 4) or in_window(19, 10, 14):
        kitchen_kw = 1.2  # mixer spike
...
# ---------- User Misconfiguration Scenario ----------

def run_user_misconfig_day(
    date: datetime,
    step_minutes: int = 1,
    ...
    output_csv_path: str = "data/daily_sim_user_misconfig.csv",
):
    """
    Scenario A5: User misconfiguration.

    - User underestimates kitchen nominal load as 0.2 kW.
    - Algorithm uses threshold = nominal + 0.1 = 0.3 kW.
    - But actual base kitchen usage is ~0.25–0.3 kW + 1.2 kW spikes.

    Expected:
    - Kitchen often exceeds threshold -> frequently forced to GRID.
    - System remains safe (no solar overload), but wastes solar potential for kitchen.
    """
    ...
    # Misconfigured kitchen threshold
    user_nominal_kitchen_kw = 0.2
    misconfig_threshold_kitchen_kw = user_nominal_kitchen_kw + 0.1  # 0.3 kW
    ...
    with open(output_csv_path, mode="w", newline="") as f:
        ...
        writer.writerow([
            ...
            "motor_pump_source",
            "geyser_source",
            "kitchen_threshold_kw",
        ])

        print("\n=== RUNNING USER MISCONFIGURATION SCENARIO ===")
        print(f"Kitchen threshold mis-set to {misconfig_threshold_kitchen_kw:.2f} kW")
        ...
        for step in range(total_steps):
            ...
            # 3) Realistic kitchen spikes
            bedroom_kw, hall_kw, kitchen_kw = add_kitchen_spikes(
                minute_of_day, bedroom_kw, hall_kw, kitchen_kw
            )

            # 4) Heavy loads same as baseline
            wm_kw = 0.0
            pump_kw = 0.0
            geyser_kw = 0.0
            ...
            # 5) Build loads with misconfigured kitchen threshold
            loads = [
                Load(
                    "bedroom_circuit",
                    power_kw=bedroom_kw,
                ...
                Load(
                    ...
                    power_kw=kitchen_kw,
                    can_run_on_solar=True,
                    high_event_threshold_kw=misconfig_threshold_kitchen_kw,
                ),
            ...
            # 7) Energy flows: inverter side vs grid side
            step_hours = step_minutes / 60.0
            ...
            writer.writerow([
                ...
                allocation["geyser"],
                round(misconfig_threshold_kitchen_kw, 3),
            ])

            current_time += timedelta(minutes=step_minutes)

        print("User misconfiguration simulation complete.")


if __name__ == "__main__":
    run_user_misconfig_day(datetime.now())
